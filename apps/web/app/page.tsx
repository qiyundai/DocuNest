...
export default async function HomePage() {
  ...
  try {
    manuals = await client.docs.listManuals.query({ tenantId: DEFAULT_TENANT_ID });
  } catch (error) {
    console.warn('Failed to fetch manuals:', error);
    // Fallback to empty array during build time
  }

  const products = manuals.reduce((acc: Record<string, any>, manual: any) => {
        ...
        {Object.values(products).map((product: any) => (
          <article key={product.id} className="rounded-lg border border-slate-800 bg-slate-900 p-4 shadow">
            ...
            <ul className="mt-3 space-y-2">
              {manuals
                .filter((manual: any) => manual.product.id === product.id)
                .map((manual: any) => (
                  <li key={manual.id}>
                    <Link
